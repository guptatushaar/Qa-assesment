...
- **Validation Notes (rejected AI “just keep Canada”):** Probed API — `UttarP`/`2134AB` invalid with `TG`; Canada + Dutch postal blocked Proceed. Kept NL + 1432AA for UI; invoice fixture remains TG/Florida/1234AA with street `Noida 130`. Centralized data in `utils.js` + JSON so UI/API no longer drift.
